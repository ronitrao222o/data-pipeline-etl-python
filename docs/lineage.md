...
## Flow
`data/raw_sales_data.csv` -> extract -> contract validation -> transform -> profile -> quality checks -> analytics -> warehouse export -> SQLite load -> monitoring -> JSON reports
...
- `artifacts/monitoring_report.json`: run health status and alert severity details
- `artifacts/warehouse/sales`: partitioned CSV files and manifest metadata

## Operational Notes
- `--dry-run` validates extract, contract, transform, profiling, quality, analytics, and monitoring without loading SQLite or warehouse exports
